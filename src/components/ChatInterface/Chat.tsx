...
const Chat: React.FC = () => {
    ...
    //             if (!prev || prev.length === 0 || !prev[0] || !prev[0].messages) {
    //                 return prev;
    //             }
    
    
    //             const newConversations = [...prev];
    //             const firstElement = {...newConversations[prev.length - 1]};
    //             const newMessages = [...firstElement.messages];
    
    //            if(newMessages.length > 1) {
    //              newMessages.splice(1, 1);
    //            }
    
    //             firstElement.messages = newMessages;
    //             newConversations[0] = firstElement
    //            return newConversations;
    
            
    //         });
    //     }
    // },[botResponseLoadingStatus,terminateResponse, conversations])
    // useEffect(() => {
    //     if(botResponseLoadingStatus === API_STATUS.SUCCESS || botResponseLoadingStatus === API_STATUS.INITIAL)
    //     setTerminateResponse(false);
    // },[botResponseLoadingStatus])
        ...
        <div className={`${styles['right-content-section']}`}>
            <div className='row'>
                ...
                <div className={(!isVendorLoggedIn && userRole === 1) ? `col-md-8` : 'col-md-12'}> 
                    ...
                    <p className={`${styles['disclaimer-message']}`}> <span className='me-2'><img src={disclaimerIcon} alt='Disclimer'/> </span>Responses are generated by AI. Exercise discretion.</p>
                </div>
            </div>
